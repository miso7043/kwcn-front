import React from 'react';
import useLanguage from '../../../hooks/useLanguage';
import ImageGalleryHor from '../../../components/ui/ImageGallery/ImageGalleryHor';
import JobAcademyNavigation from '../../../components/features/information/JobAcademyNavigation';
import { jobAcademyStyles } from './jobAcademyStyles';
import * as job1ko from './locales/Job1_ko';
import * as job1en from './locales/Job1_en';


type SimpleInfo = {
  title: string;
  description: string;
};

const Job1: React.FC = () => {
  const { language } = useLanguage();
  const job1 = language === 'en' ? job1en : job1ko;
  const { job1Text } = job1;

  const galleryImages = [
    { src: '/imgs/jobacademy/lecture1-WorkBC1.jpg', alt: 'WorkBC 1', title: '' },
    { src: '/imgs/jobacademy/lecture1-WorkBC2.jpg', alt: 'WorkBC 2', title: '' },
    { src: '/imgs/jobacademy/lecture1-WorkBC3.jpg', alt: 'WorkBC 3', title: '' },
    { src: '/imgs/jobacademy/lecture1-WorkBC4.jpg', alt: 'WorkBC 4', title: '' },
  ];

  const workBcQuizzes = job1Text.quizzes.map((quiz: SimpleInfo, idx: number) => ({
    ...quiz,
    url: [
      'https://careerdiscoveryquizzes.workbc.ca/form/abilities-quiz',
      'https://careerdiscoveryquizzes.workbc.ca/form/work-preferences-quiz',
      'https://careerdiscoveryquizzes.workbc.ca/form/interests-quiz',
    ][idx],
  }));

  const certificates = job1Text.certificates.map((cert: SimpleInfo, idx: number) => ({
    ...cert,
    url: [
      'http://www.foodsafe.ca/',
      '#',
      'http://www.redcross.ca/training-and-certification',
      'https://www.worksafebc.com/en/health-safety/education-training-certification/forklift-operator',
      'https://www.bccsa.ca/Standardized-Traffic-Control-Training.html',
      'https://www.servingitright.com/',
      '#',
      'https://gv.ymca.ca/Blogs/CCRR-Blog/August-2017/Looking-for-ECE-workshops',
      '#',
    ][idx],
  }));

  const helpfulResources = job1Text.resources.map((res: SimpleInfo, idx: number) => ({
    ...res,
    url: [
      'https://www.workbc.ca/',
      'https://www.jobbank.gc.ca/explorecareers',
      'http://www.bc211.ca/',
      'http://newtobc.ca/',
      'http://www.cic.gc.ca/english/newcomers/services/index.asp',
      'http://www.govolunteer.ca/',
      'http://www.worksafebc.com/',
    ][idx],
  }));

  return (
    <div className="min-h-screen">
      {/* 헤더 섹션 */}
      <div className="relative overflow-hidden">
        <div
          className="relative bg-cover bg-center bg-no-repeat min-h-[300px] flex flex-col justify-center items-center"
          style={{ backgroundImage: "url('/imgs/jobacademy/jobTitleBack.webp')" }}
        >
          <h1 className="pt-10 text-6xl md:text-8xl font-black text-white drop-shadow-2xl">
            {job1Text.title}
          </h1>
        </div>
      </div>

      {/* Lecture 1 섹션 */}
      <div className="relative max-w-[90%] md:max-w-[80%] mx-auto rounded-lg overflow-hidden mt-3">
        <img
          src="/imgs/jobacademy/lectureBack.webp"
          alt="Lecture Background"
          className="w-full h-auto object-contain"
        />

        <div className={jobAcademyStyles.absolutePositions.lectureNumberPos}>
          <h2 className="font-bold text-black drop-shadow-lg text-[clamp(1rem,2.5vw,3rem)]">Lecture 1</h2>
        </div>
        <div className={`absolute left-[66%] ${language === 'ko' ? 'top-[38%]' : 'top-[26%]'}`}>
          <h2 className="font-bold text-white drop-shadow-lg text-[clamp(0.8rem,2.5vw,3rem)]">
            {job1Text.lectureSubtitle.split('\n').map((line: string, index: number) => (
              <React.Fragment key={index}>
                {line}
                {index < job1Text.lectureSubtitle.split('\n').length - 1 && <br />}
              </React.Fragment>
            ))}
          </h2>
        </div>
      </div>

      <div className={jobAcademyStyles.containers.centerContainer}>
        {/* Lecture 1 계획 */}
        <section className={jobAcademyStyles.section.basic}>
          <h2 className={jobAcademyStyles.sectionTitle}>
            {job1Text.lecturePlan}
            <span className={jobAcademyStyles.sectionTitleAccent}></span>
          </h2>
          <div className={jobAcademyStyles.containers.lightBox}>
            <ul className={jobAcademyStyles.spacing.itemSpacing}>
              <li className={jobAcademyStyles.listStyles.checkItem}>
                <span className={jobAcademyStyles.listStyles.checkMark}>✓</span>
                {job1Text.planItem1}
              </li>
              <li className={jobAcademyStyles.listStyles.checkItem}>
                <span className={jobAcademyStyles.listStyles.checkMark}>✓</span>
                {job1Text.planItem2}
              </li>
            </ul>
          </div>

          <h3 className="bg-stone-600 text-red-400 text-2xl italic text-center my-6 p-4 border-l-4 border-red-500 rounded">
            {job1Text.tedQuote}
          </h3>

          {/* TED 영상 삽입 */}
          <div className="my-8 flex justify-center">
            <div className="w-full max-w-4xl">
              <div className="relative w-full" style={{ paddingBottom: '56.25%' /* 16:9 aspect ratio */ }}>
                <iframe
                  className="absolute top-0 left-0 w-full h-full rounded-lg shadow-lg"
                  src="https://www.youtube.com/embed/iKHTawgyKWQ"
                  title="Why you will fail to have a great career | Larry Smith | TEDxUW"
                  frameBorder="0"
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                  allowFullScreen
                ></iframe>
              </div>
            </div>
          </div>
        </section>

        <section className={jobAcademyStyles.section.basic}>
          <div className="text-gray-900 leading-relaxed text-lg space-y-4">
            <p> 
              {job1Text.introduction.paragraph1}
            </p>
            <p>
              {job1Text.introduction.paragraph2}
            </p>
          </div>
          <img
            src={job1Text.imgLikeGood}
            alt="Like Good"
            className="p-[10%] h-auto object-contain"
          />
        </section>

        {/* Work BC 웹사이트 */}
        <section className={jobAcademyStyles.section.basic}>
          <h2 className={jobAcademyStyles.sectionTitle}>
            {job1Text.workBcSection.title}
            <span className={jobAcademyStyles.sectionTitleAccent}></span>
          </h2>
          <div className="text-gray-900 leading-relaxed text-lg space-y-4">
            <p>
              {job1Text.workBcSection.description1}
            </p>
            <p>{job1Text.workBcSection.description2}</p>
          </div>

          {/* 이미지 갤러리 */}
          <section className={jobAcademyStyles.section.basic}>
            <h2 className={jobAcademyStyles.sectionTitle}>
              {job1Text.galleryTitle}
              <span className={jobAcademyStyles.sectionTitleAccent}></span>
            </h2>
            <ImageGalleryHor images={galleryImages} />
          </section>

          <div className="mt-8">
            <h4 className={`${jobAcademyStyles.headings.h3} mb-4`}>{job1Text.workBcSection.quizzesTitle}</h4>
            <div className={`${jobAcademyStyles.grids.twoThreeColumn} mt-8`}>
              {workBcQuizzes.map((quiz, index) => (
                <a
                  key={index}
                  href={quiz.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block p-6 bg-gradient-to-r from-blue-500 to-blue-600 text-center text-white rounded-lg hover:-translate-y-1 hover:shadow-lg transition-all duration-300 hover:from-blue-600 hover:to-blue-700"
                >
                  <h5 className={jobAcademyStyles.specialText.cardTitle}>{quiz.title}</h5>
                  {quiz.description && <p className="text-sm opacity-90">{quiz.description}</p>}
                </a>
              ))}
            </div>
          </div>
        </section>

        {/* 적성과 특기, 캐나다 구직활동 */}
        <section className={jobAcademyStyles.section.basic}>
          <h2 className={jobAcademyStyles.sectionTitle}>
            {job1Text.careerSection.title}
            <span className={jobAcademyStyles.sectionTitleAccent}></span>
          </h2>
          <div className="text-gray-900 leading-relaxed text-lg space-y-4">
            <p>
              {job1Text.careerSection.paragraph1}
            </p>
            <p>
              {job1Text.careerSection.paragraph2}
            </p>
          </div>

          <img
            src={job1Text.imgLec2Diagram}
            alt="Lecture 2 Diagram"
            className="my-[4rem] mx-auto h-auto object-contain "
          />

          <div className="mt-8">
            <h4 className={`${jobAcademyStyles.headings.h3} mb-6`}>{job1Text.careerSection.improvementTitle}</h4>

            <div className={`${jobAcademyStyles.containers.lightBox} mb-4 border-l-4 border-blue-500`}>
              <h5 className={`${jobAcademyStyles.headings.h5} mb-4`}>{job1Text.careerSection.languageSkills.title}</h5>
              <ul className={jobAcademyStyles.listStyles.bulletList}>
                <li>{job1Text.careerSection.languageSkills.items[0]}</li>
                <li>{job1Text.careerSection.languageSkills.items[1]} (<a href="https://www.linkedin.com/" target="_blank" rel="noopener noreferrer" className={jobAcademyStyles.textColors.link}>linkedin.com</a>)</li>
              </ul>
            </div>

            <div className={`${jobAcademyStyles.containers.lightBox} mb-4 border-l-4 border-blue-500`}>
              <h5 className={`${jobAcademyStyles.headings.h5} mb-4`}>{job1Text.careerSection.offlineActivities.title}</h5>
              <ul className={jobAcademyStyles.listStyles.bulletList}>
                <li>
                  <strong className={jobAcademyStyles.textColors.grayDark}>Toastmasters</strong> - {job1Text.careerSection.offlineActivities.toastmasters}
                  (<a href="https://www.toastmasters.org/" target="_blank" rel="noopener noreferrer" className={jobAcademyStyles.textColors.link}>toastmasters.org</a>)
                </li>
                <li>
                  <strong className={jobAcademyStyles.textColors.grayDark}>Meet Up</strong> - {job1Text.careerSection.offlineActivities.meetup}
                  (<a href="https://www.meetup.com/" target="_blank" rel="noopener noreferrer" className={jobAcademyStyles.textColors.link}>meetup.com</a>)
                </li>
              </ul>
            </div>

            <div className={`${jobAcademyStyles.containers.lightBox} mb-4 border-l-4 border-blue-500`}>
              <h5 className={`${jobAcademyStyles.headings.h5} mb-4`}>{job1Text.careerSection.volunteering.title}</h5>
              <ul className={jobAcademyStyles.listStyles.bulletList}>
                <li>
                  {job1Text.careerSection.volunteering.item1}
                  (<a href="https://www.govolunteer.ca/" target="_blank" rel="noopener noreferrer" className={jobAcademyStyles.textColors.link}>govolunteer.ca</a>)
                </li>
                <li>{job1Text.careerSection.volunteering.item2}</li>
              </ul>
            </div>

            <div className={`${jobAcademyStyles.containers.lightBox} mb-4 border-l-4 border-blue-500`}>
              <h5 className={`${jobAcademyStyles.headings.h5} mb-4`}>{job1Text.careerSection.supportNetwork.title}</h5>
              <ul className={jobAcademyStyles.listStyles.bulletList}>
                <li>{job1Text.careerSection.supportNetwork.item}</li>
              </ul>
            </div>
          </div>

          <div className={jobAcademyStyles.section.highlightInBox}>
            <h4 className={`${jobAcademyStyles.headings.h5} mb-4`}>{job1Text.careerSection.keyPoints.title}</h4>
            <ul className={jobAcademyStyles.spacing.itemSpacing}>
              <li className="relative pl-6 text-black leading-relaxed">
                <span className="absolute left-0">💡</span>
                {job1Text.careerSection.keyPoints.point1}
              </li>
              <li className="relative pl-6 text-black leading-relaxed">
                <span className="absolute left-0">💡</span>
                {job1Text.careerSection.keyPoints.point2}
              </li>
            </ul>
          </div>
        </section>

        {/* 자격증 섹션 */}
        <section className={jobAcademyStyles.section.basic}>
          <h2 className={jobAcademyStyles.sectionTitle}>
            {job1Text.certificatesSection.title}
            <span className={jobAcademyStyles.sectionTitleAccent}></span>
          </h2>
          <div className="text-gray-900 leading-relaxed text-lg space-y-4">
            <p>{job1Text.certificatesSection.description1}</p>
            <p>
              {job1Text.certificatesSection.description2}
            </p>
            <p>
              {job1Text.certificatesSection.description3}
            </p>
          </div>

          <div className={`${jobAcademyStyles.grids.twoThreeColumn} mt-8`}>
            {certificates.map((cert, index) => (
              <div key={index} className="bg-slate-300 text-black p-6 rounded-xl hover:-translate-y-1 hover:shadow-lg transition-all duration-300">
                <h4 className={jobAcademyStyles.specialText.cardTitle}>{cert.title}</h4>
                <p className={jobAcademyStyles.specialText.cardDescription}>{cert.description}</p>
                {cert.url !== '#' && (
                  <div className="text-center">
                    <a href={cert.url} target="_blank" rel="noopener noreferrer" className={jobAcademyStyles.buttons.primary}>
                      {job1Text.certificatesSection.viewDetails}
                    </a>
                  </div>
                )}
              </div>
            ))}
          </div>
        </section>

        {/* 도움 기관 및 정보 */}
        <section className={jobAcademyStyles.section.basic}>
          <h2 className={jobAcademyStyles.sectionTitle}>
            {job1Text.resourcesSection.title}
            <span className={jobAcademyStyles.sectionTitleAccent}></span>
          </h2>
          <div className={`${jobAcademyStyles.grids.twoThreeColumn} mt-8`}>
            {helpfulResources.map((resource, index) => (
              <a
                key={index}
                href={resource.url}
                target="_blank"
                rel="noopener noreferrer"
                className="block bg-gradient-to-r from-emerald-300 to-emerald-400 text-zinc-800 p-6 rounded-xl hover:-translate-y-1 hover:shadow-lg transition-all duration-300"
              >
                <h4 className="text-xl font-bold mb-3">{resource.title}</h4>
                <p className={jobAcademyStyles.specialText.emphasis}>{resource.description}</p>
              </a>
            ))}
          </div>
        </section>

        {/* Job Academy 네비게이션 */}
        <section className={jobAcademyStyles.section.basic}>
          <JobAcademyNavigation currentJobId={1} />
        </section>
      </div>
    </div>
  );
};

export default Job1;
